...
class VideoConverter(object):
    ...
    def process_video(self, input_img, options): 
        ...
        detections = self.fdetect.detect_face(image)
        ...
        for idx, det in enumerate(detections):
            ...
            try:
                ...
                src_landmarks = FaceExtractor.get_src_landmarks(x0, x1, y0, y1, lms)
                tar_landmarks = FaceExtractor.get_tar_landmarks(det_face_im)

                # align detected face
                aligned_det_face_im = FaceExtractor.landmarks_match_mtcnn(det_face_im, src_landmarks, tar_landmarks)
                ...
                # reverse alignment
                rev_aligned_det_face_im = FaceExtractor.landmarks_match_mtcnn(r_im, tar_landmarks, src_landmarks)
                rev_aligned_det_face_im_rgb = FaceExtractor.landmarks_match_mtcnn(r_rgb, tar_landmarks, src_landmarks)
                rev_aligned_mask = FaceExtractor.landmarks_match_mtcnn(r_a, tar_landmarks, src_landmarks)
